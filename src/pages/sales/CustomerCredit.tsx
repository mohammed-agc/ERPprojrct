...
import {
  Search, AlertTriangle, ShieldCheck, FileText, Clock, Settings, RotateCw,
} from "lucide-react";
import {
  customerSettlementService, fmtSAR, fmtDate, SETTLEMENT_LABEL,
  type CustomerCreditRow,
} from "@/services/erp/customerSettlement";
import { CustomerPolicyDialog } from "@/components/erp/CustomerPolicyDialog";

/**
 * Customer Credit Governance — full parity with /purchasing/credit.
 *
 * Shows credit limits, exposure, overdue balances, and links to the
 * authoritative customer statement (/ar/:id).
 */
export default function CustomerCredit() {
  const [q, setQ] = useState("");
  const [rows, setRows] = useState<CustomerCreditRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [policy, setPolicy] = useState<CustomerCreditRow | null>(null);

  const load = () => {
    setLoading(true);
    customerSettlementService.listCustomerCredit()
      .then(setRows)
  ...
  return (
    <div>
      ...
      <div className="sticky top-[64px] z-10 bg-background/95 backdrop-blur border border-border rounded-lg p-3 mb-3 flex items-center gap-2">
        ...
        <div className="text-xs text-muted-foreground ml-auto">{filtered.length} عميل</div>
      </div>

      {loading ? (
        <div className="text-center text-muted-foreground py-10 text-sm">جارٍ التحميل…</div>
      ) : filtered.length === 0 ? (
        <EmptyState title="لا يوجد عملاء" description="لا توجد بيانات ائتمان عملاء." />
      ) : (
              ...
              <div key={c.id} className="bg-card border border-border rounded-lg p-4">
                <div className="flex items-start justify-between mb-3 gap-2">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <Button size="sm" variant="outline" className="h-7 px-2 text-[12px]" onClick={() => setPolicy(c)}>
                      <Settings className="h-3 w-3 ml-1" /> السياسة
                    </Button>
                  </div>
                </div>
