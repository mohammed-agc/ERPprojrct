...
export default function AllocationConfirmationDetail() {
  const { id = "" } = useParams();
  const cc = useMemo(() => allocationService.getConfirmation(id), [id]);
  if (!cc) return <div className="p-6 text-sm text-muted-foreground">الوثيقة غير موجودة</div>;
  const sup = purchasingService.getSupplier(cc.supplier_id);
  const po = purchasingService.getPO(cc.po_id);

  return (
    <div className="space-y-3">
      <PageHeader
        title={`وثيقة تأكيد ${cc.code}`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-3">
        <div className="space-y-3">
          <div className="bg-card border border-border rounded-lg p-3 text-xs grid grid-cols-2 gap-2">
            ...
            <div><span className="text-muted-foreground">أمر الشراء:</span> <span className="font-mono">{po?.code}</span></div>
            ...
            <div className="col-span-2"><span className="text-muted-foreground">الفاتورة المرتبطة:</span> {cc.invoice_id ? <span className="text-success">مرتبطة</span> : <span className="text-muted-foreground">لم تُربط بعد</span>}</div>
