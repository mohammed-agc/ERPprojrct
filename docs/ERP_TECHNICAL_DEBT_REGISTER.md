...
- **G4 — Payment Guard Table Mismatch (Confirmed with live data evidence; DEBT-012 expansion, not a separate debt):** `can_cancel_sales_invoice` checks `sales_payments` for the paid amount, but real payments post through the `payments` table (which carries the `create_payment_journal_entry` AFTER INSERT trigger). Live evidence: INV-2026-0002 had a real payment of 57,500 in `payments` (0 rows in `sales_payments`), with payment JE-2026-0006 (Dr cash / Cr AR); the guard read `v_paid=0` and allowed cancellation — which is why a fully-paid invoice was cancelled and left the AR residual. DEBT-012 now has four dimensions: (1) posting defect — no GL treatment for prior payment/settlement on cancel; (2) settlement/open-allocation guard gap — guard does not inspect `open_item_allocations`; (3) payment guard table mismatch — guard checks `sales_payments` not `payments` (G4); (4) result — paid/settled invoices can be cancelled, leaving AR credit residuals. Corrected guard must inspect `payments` and active unreversed `open_item_allocations`, not `sales_payments`. Not Implemented.
- **Phase 0B workflow gaps (G1–G3) resolved by read-only evidence; Phase 0B = Seed-via-Workflow Final Review Version / Not Executed:** G1 — the staging seed invoice is a simple issued invoice with no vehicle lines (revenue JE only; no COGS/inventory; ZINV-TEST pattern). G2 — payment seed uses the `payments` table, fetches the generated JE (`source_type='sales_payment'`, `source_id=payments.id`, exactly one) and passes it explicitly to `create_allocation`; a NULL `journal_entry_id` is forbidden (reproduces DEBT-010). G3 — T7 (ambiguous/multi-line settlement) is a fixture/unit test, not a DB seed, because `create_partner_settlement` only builds clean two-line entries and hand-building a posted multi-line JE would pollute the ledger. Details in the Staging Execution Plan §4a. Remaining blocker is environmental (staging not confirmed).

#### Session Evidence Update — 2026-07-06 (Remediation Design Review Session)

This session performed live reads of the cancellation functions and confirmed/refined DEBT-012, plus produced remediation designs for five candidates. All design/review only — no code change, no DB writes, no execution.

**DEBT-012 — Dimension 1 wording refined (design review `docs/SARAT_REMEDIATION_DEBT012_DESIGN_REVIEW.md`, commit 8bc9f68):**
A live read of `cancel_sales_invoice` (via `pg_get_functiondef`, 2026-07-06) shows the function is in fact **mature** and reverses a great deal: F5.2 revenue reversal (mirror clone + `reverses_entry_id` + marks original `is_reversed`), F5.3 COGS reversal (keyed on an actual active COGS JE, not the `cogs_posted` flag), F5.4 inventory reversal (vehicle Per-VIN assignment vs part cumulative increment), F5.5 open-item allocation reversal (`REV-*` rows **with `reverses_allocation_id`**, filtering `reverses_allocation_id IS NULL`), plus a posted Credit Note, engine-based numbering, and governance log. So the accurate statement of Dimension 1 is **not** "no reversing GL." It is: `cancel_sales_invoice` reverses revenue, COGS, inventory, and allocations, but does nothing about a prior cash payment — it neither reverses nor reclassifies it — because it is built on the assumption that no payment exists (an assumption the broken guard lets through). The revenue side is unwound; the cash side (Dr 1111 / Cr 1131 from the original receipt) is left stranded, leaving an AR credit in the GL with no offset. The load-bearing fix is the guard source (must read `payments` + active unreversed `open_item_allocations`, not `sales_payments`).

**DEBT-012 — cross-reference to DEBT-011 (positive finding):** `cancel_sales_invoice` F5.5 already respects `reverses_allocation_id` when inserting its `REV-*` reversal rows (it filters `reverses_allocation_id IS NULL` on the source set). This means the cancellation flow is on the **correct** side of DEBT-011 for its own reversal rows, and any DEBT-012 remediation must not regress this behaviour.

**DEBT-012 — remediation review recommendation:** the existing design (`ERP_DEBT012_REMEDIATION_DESIGN.md` + `ERP_DEBT012_STAGING_EXECUTION_PLAN.md`) remains directionally correct. The review adds: (1) restate Dimension 1 with the accurate wording above; (2) treat the guard source fix as the first, load-bearing change; (3) make the block-vs-handle choice explicit and sequence it (guard fix → Customer Receipt Reversal primitive → optional inline F5.7); (4) preserve every mature behaviour already in `cancel_sales_invoice` (F5.2–F5.6) and the guard's non-payment checks (posted-CN idempotency, REVENUE_JE_MISSING, COGS_JE_MISSING, VEHICLE_DELIVERED). DEBT-012 status unchanged: Confirmed debt, High, Active GL Impact, Not Implemented.

**Candidate remediation designs added this session (design only — candidates NOT promoted):**
- **R-RR2b** (`docs/SARAT_REMEDIATION_R_RR2B_CASHFLOW_DESIGN.md`, commit ec1d1d5): live read of `cashFlow` (accounting.ts:1105) shows a 5-criteria heuristic, not an exclusive hardcode; the real cash accounts are 1111 + 1121 (under CUSTOMER_PAYMENT_RECEIVE / SUPPLIER_PAYMENT_PAY), not 1101/1102. Fix = Option C hybrid (CASH_AND_EQUIVALENTS key if present, else union of payment keys; drop names + literals).
- **R-FA2** (`docs/SARAT_REMEDIATION_R_FA2_FUTURE_PERIOD_GUARD_DESIGN.md`, commit 08245e9): live read of `run_monthly_depreciation(p_period text)` shows a mature function; the only gap is no future-period check. Fix = a single guard after `v_period_date` (`IF v_period_date > date_trunc('month', CURRENT_DATE) THEN RAISE`), strict boundary, RAISE not silent-skip.
- **R-AP1/R-AP2/R-AP3** (`docs/SARAT_REMEDIATION_R_AP1_AP2_AP3_APPROVAL_DESIGN.md`, commit 8ebd76f): live read of `approve_request` + `approval_workflow_steps` (11 columns) shows a mature function (SoD, admin override logged, multi-step, document posting); the shared root is defined-but-unconsumed columns. R-AP2 (respect `approver_type` role-vs-user → `approver_id`) → R-AP1 (enforce `approval_mode='all'`) → R-AP3 (SLA sweep on `timeout_hours` → `escalate_to`). This is the same finding as CANDIDATE-004 (AW2 mode=ALL + escalation), now with live evidence and a design.
- **R-Z1** (`docs/SARAT_REMEDIATION_R_Z1_ICV_ENVIRONMENT_DESIGN.md`, commit 9dfee4f): metadata-only read of `zatca_icv_counter` (keyed on `company_id` only, no `environment`) + atomic `next_zatca_icv(p_company)`. Fix = Option A (add `environment` column, PK `(company_id, environment)`, `p_environment` param, callers pass environment explicitly; keep atomic pattern). ZATCA needs per-environment contiguous ICV chains.
- **R-Z2** (`docs/SARAT_REMEDIATION_R_Z2_CREDENTIAL_LAST_USED_DESIGN.md`, commit 851fd31): metadata-only read of `zatca_credentials` (21 cols) shows `last_used_at` **exists** (timestamptz, nullable) but is never populated — a defined-but-unconsumed column, not a missing one. Fix = Option B (a tiny `touch_zatca_credential(p_credential_id)` that stamps `last_used_at = now()` after successful use; metadata-only write, never touches `secret_encrypted`).

**Governing constraints (unchanged):** staging not confirmed → treat as production → no DB writes, no execution, no remediation implementation. Evidence Level for all the above = VERIFIED (live DB/code reads this session). No candidate promoted; no debt re-classified.
